...
const COMPARE_DATA: ComparePageData[] = [
  {
    slug: "guessing",
    maturityKey: "guess",
...
export default async function ComparePage({ params }: Props) {
  ...
  const maturityLine = MATURITY_LINE[data.maturityKey];
  const maturityNote = MATURITY_NOTE[data.maturityKey];
    ...
    <div className="pseo-page">
      ...
      <main className="pseo-main pseo-article">
        ...
        <div className="pseo-hero">
          <span className="pseo-eyebrow">Comparison</span>
          <h1>{data.title}</h1>
          <p className="pseo-definition pseo-maturity-line">{maturityLine}</p>
          <p className="pseo-lead">{maturityNote}</p>
        </div>
        ...
        <article className="pseo-body">
          ...
          <section>
            <h2>What this approach does well</h2>
        ...
        <section className="pseo-section pseo-cta-section">
          ...
          <p>
            ...
            app count — scored per storefront, in seconds.
          </p>
        ...
        <section className="pseo-section">
          ...
          <ul className="pseo-related-list">
            {COMPARE_DATA.filter((d) => d.slug !== slug).map((d) => (
              <li key={d.slug}>
                <Link href={`/compare/${d.slug}`}>{d.title} →</Link>
              </li>
            ))}
          </ul>
        </section>
