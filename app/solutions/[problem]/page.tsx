...
const SOLUTION_DETAILS: SolutionDetail[] = [
  {
...
export default async function SolutionDetailPage({ params }: Props) {
  ...
  const faq = faqSchema(detail.faq);
      ...
      <main className="pseo-main pseo-article">
        ...
        <div className="pseo-hero">
          <div className="pseo-store-badge">
            <span className="pseo-proof-badge">{fix.proof}</span>
          </div>
          <h1>{detail.title}</h1>
          <p className="pseo-definition">{detail.subtitle}</p>
          <div className="pseo-problem-box">
            <strong>The Problem:</strong> {fix.problem}
            <br />
            <strong>The Fix:</strong> {fix.fix}
          </div>
        </div>
        ...
        <article className="pseo-body">
          ...
          <section>
            <h2>How ASOGrade solves this</h2>
            <ol className="pseo-steps">
              {detail.howItWorks.map((step, sIdx) => (
                <li key={sIdx}>{step}</li>
              ))}
            </ol>
          </section>
        ...
        <section className="pseo-section pseo-cta-section">
          <h2>Ready to solve this for your app?</h2>
          <p>
            Join indie developers scoring App Store keywords by demand and
            difficulty across 109 storefronts.
          </p>
        ...
        <section className="pseo-section">
          <h2>Other ASO solutions</h2>
          <ul className="pseo-related-list">
            {SOLUTION_DETAILS.filter((d) => d.slug !== detail.slug).map((d) => (
              <li key={d.slug}>
                <Link href={`/solutions/${d.slug}`}>{d.title} →</Link>
              </li>
            ))}
          </ul>
        </section>
