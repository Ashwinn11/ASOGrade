...
for (const entity of allEntities) {
  ...
  if (entity.category === "storefront") {
    ...
    titleCandidates = [
      ...
      fitDescription(entity.title, 55),
      "ASOGrade Guide",
    ];
    descCandidates = [entity.description];
  } else if (entity.category === "persona") {
    titleCandidates = [
      `${entity.metaTitle ?? entity.title} | ASOGrade`,
      entity.metaTitle ?? entity.title,
      fitDescription(entity.title, 55),
    ...
    descCandidates = [entity.description];
  } else if (entity.category === "solution") {
    titleCandidates = [
      `${entity.metaTitle ?? entity.title} | ASOGrade`,
      entity.metaTitle ?? entity.title,
      fitDescription(entity.title, 55),
    ...
    descCandidates = [entity.description];
  } else if (entity.category === "localization") {
    titleCandidates = [
      `${entity.metaTitle ?? entity.title} | ASOGrade`,
      entity.metaTitle ?? entity.title,
      fitDescription(entity.title, 55),
...
for (const entity of allEntities) {
  ...
  if (entity.category === "guide") {
    ...
    assert(Boolean(articleNode.image && articleNode.image.url), `Guide [${entity.slug}] Article schema missing image`);
  }
...
async function main() {
  ...
  if (errors.length === 0) {
    ...
    console.log("  The programmatic SEO system is verified and safe for 100,000+ pages!");
    console.log("=================================================\n");
    process.exit(0);
  } else {
